...
class ThumbnailMakerTest(unittest.TestCase):
    ...
    def test_non_dry_main_resolves_video_before_scene_helpers_with_mocks(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                mock.patch("astrid.packs.builtin.executors.scenes.run.detect_scenes", side_effect=fake_detect),
                mock.patch("astrid.packs.builtin.executors.scenes.run.write_outputs"),
                mock.patch("astrid.packs.builtin.executors.shots.run.build_shots", side_effect=fake_build_shots),
                mock.patch("astrid.packs.builtin.executors.generate_image.run.main", side_effect=fake_generate),
            ):
                result = thumbnail_maker.main(
                    [
                        "--video",
    ...
    def test_non_dry_generation_uses_generate_image_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch("astrid.packs.builtin.executors.generate_image.run.main", side_effect=fake_generate) as generate_main:
