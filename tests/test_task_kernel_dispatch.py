...
from artagents.core.executor import cli as executor_cli
from artagents.core.executor import runner as executor_runner
from artagents.core.executor.runner import ExecutorRunRequest, ExecutorRunResult, ExecutorRunnerError
from artagents.core.orchestrator.runner import OrchestratorRunRequest, OrchestratorRunnerError, run_orchestrator
...
def test_pipeline_dispatch_calls_top_gate_and_executor_reentry(
    ...
    with patch("artagents.core.task.gate.gate_command", wraps=task_gate.gate_command) as gate_spy:
...
def test_orchestrator_runner_rejects_before_project_run_side_effects(
    ...
    with patch("artagents.core.task.gate.gate_command", wraps=task_gate.gate_command) as gate_spy:
        with pytest.raises(OrchestratorRunnerError, match="artagents next --project demo"):
...
def test_executor_runner_rejects_before_project_run_side_effects(
    ...
    with patch("artagents.core.task.gate.gate_command", wraps=task_gate.gate_command) as gate_spy:
        with pytest.raises(ExecutorRunnerError, match="artagents next --project demo"):
