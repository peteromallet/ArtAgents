...
|----------------------------------------------------------|-----------------------------|--------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------|-----------------------------------------------------------------------|
| `external.fal_foley` (one executor)                      | `astrid-pack-fal-foley`     | `https://github.com/peteromallet/astrid-pack-fal-foley` | `fal-client>=0.7.0` (already pinned in `requirements.txt`). Adapter currently reaches into `astrid.packs.builtin.logo_ideas.run` for the shared `fal` HTTP helpers (`FAL_QUEUE_URL`, `_http_get_bytes`, `_http_post_json`, `poll_fal_result`) and `astrid.packs.builtin.vary_grid.run._load_env_var`. The extraction must vendor or repackage those helpers (see §2.4). | `FAL_KEY` (read via `_load_env_var`).                       | Subprocess `python -m astrid.packs.external.executors.fal_foley.run`. |
