# Claude Code Stop hook — `artagents hook stop`
...
that re-prints `artagents next` (preamble + current step) into the stream
...
`artagents hook stop` is a no-op unless an active run exists somewhere it can
...
2. **projects-root scan** — if no ancestor matched, iterate the projects
   ...
   `~/Documents/reigh-workspace/artagents-projects/`) and pick every
   subdirectory whose name is a valid project slug and that contains an
   `active_run.json`.

For each discovered slug (sorted), the hook re-prints `artagents next`
