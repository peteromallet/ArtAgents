...
This is a focused 10-working-day sprint to ship the ArtAgents thread, variant, and iteration-video layer.
...
The target demo fixture is `runs/artagents_logo_v3`.
...
Top risk 2 is lock contention under parallel runs; mitigation is `fcntl` locking with a 30-second timeout and stale-lock guidance that tells the user to verify no ArtAgents writer is active before manual lock-file removal. No lock-repair command ships in v1.
...
Definition of done: all post-wrapper runs have v1 metadata, the week-1 SD-037 coverage is green as trimmed, full pytest is green, `runs/artagents_logo_v3` renders all five SD-022 outputs, `--no-content` works for thread/report views, and the Day-10 demo checklist is signed.
...
Day-10 demo: produce a 60-second `iteration.mp4` against `runs/artagents_logo_v3`, screen-share it, verify the report and quality JSON, and capture stdout proving every run showed the correct `[thread]` prefix before render.
...
- **SD-003** — Store the thread map in `.artagents/threads.json` with `fcntl` lock, atomic replace, and `.bak` rotation. _load_bearing: true_
...
- **Day 10** lands SD-035 acceptance: dogfood against `runs/artagents_logo_v3`, full integration pass, 60-second demo render, and observable failure-mode checks.
...
- Add `artagents/threads/ids.py` with monotonic, 26-character Crockford ULID generation for `run_id`, `thread_id`, and `group_id`.
- Add `artagents/threads/schema.py` with `schema_version: 1` constants and typed v1 shapes for `threads.json`, `run.json`, `selections.jsonl`, `groups.json`, and the provenance block.
...
- Add `artagents/threads/index.py` for `.artagents/threads.json` with a sidecar lock file, `fcntl.flock`, a 30-second acquire timeout, tmp-write + fsync + directory fsync + `os.replace`, and rotated `.bak`.
- On lock timeout, emit actionable guidance to verify no ArtAgents writer is active before manually removing a stale lock file. Do not name or ship a lock-repair command in v1.
...
- `.artagents/threads.json` is created on first write with `schema_version: 1`, `threads`, and `active_thread_id`.
...
- Eligible executor runs write `run.json`, update `.artagents/threads.json`, and finalize `ended_at` and `returncode`.
...
- Add `artagents/threads/attribute.py` with the five-branch SD-006 decision function, evaluated at run start under the index lock:
...
- Add `artagents/threads/prefix.py` for v1 stdout prefix lines per SD-018 and OQ-3.
...
- Add a footer line to `executors inspect` and `orchestrators inspect` output naming the active thread and pointing to `python3 -m artagents thread show @active`.
...
- Add `python3 -m artagents thread archive` and `thread reopen` in the morning carryover slot.
...
- Add `artagents/threads/selections.py` to append selection events to `.artagents/threads/<thread-id>/selections.jsonl` with line-buffered atomic appends and no fcntl lock per SD-042.
- Add `artagents/threads/groups.py` to maintain denormalized `.artagents/threads/<thread-id>/groups.json`.
...
- Add `python3 -m artagents thread backfill` per SD-034. Backfill scans existing `runs/*/` directories without `run.json`, computes hashes, and adopts orphan directories into auto-named threads clustered by hash-graph connected components, not mtime.
...
- `hype.metadata.json` contains the provenance block and remains useful if `.artagents/threads.json` is deleted.
...
- Add `artagents/modalities/__init__.py` and the registry loader per SD-025.
- Add exactly three renderer modules:
  - `artagents/modalities/image_grid.py`
  - `artagents/modalities/audio_waveform.py`
  - `artagents/modalities/generic_card.py`
...
- `python3 -m artagents modalities list` shows exactly `image_grid`, `audio_waveform`, and `generic_card`.
- `python3 -m artagents modalities inspect generic_card` shows it is a fallback renderer.
...
- Direct `python3 -m artagents executors run iteration.prepare ...` invocations cannot bypass the cap.
- Add the SD-043 cache at `.artagents/iteration_cache/<run_id>__<summarizer_model_version>.json`.
...
- Cache files use `.artagents/iteration_cache/<run_id>__<summarizer_model_version>.json`.
...
- Enforce SD-024 quality floor: if `data_quality < 0.6`, refuse with actionable `python3 -m artagents thread backfill ...` commands for unresolved-producer runs.
...
- `python3 -m artagents orchestrators inspect builtin.iteration_video --json` exposes the orchestrator metadata and stage file.
...
Dogfood the sprint against `runs/artagents_logo_v3` and make the demo observable, not anecdotal.

**Deliverables**

- Run Day-5 `thread backfill` against `runs/artagents_logo_v3` if it has not already been adopted.
- Run `python3 -m artagents thread show @active` before any iteration-video command in the dogfood session.
- Run `iteration_video inspect <thread>` and capture the inspect output before render.
- Produce a 60-second `iteration.mp4` against `runs/artagents_logo_v3`.
...
- `runs/artagents_logo_v3` dogfood reaches `data_quality >= 0.6` without penalizing valid roots.
...
- `tests/test_iteration_video_dogfood.py`: smoke fixture for the five SD-022 outputs against `runs/artagents_logo_v3` or its compact test fixture equivalent.
...
1. **Day 1: atomic index + `xxhash`.** The sprint needs locked `.artagents/threads.json`, ULIDs, repo-relative paths, and `xxhash>=3.4` before any run metadata can be trusted.
...
10. **Day 10: dogfood + 60-second demo + failure-mode acceptance.** `runs/artagents_logo_v3` must render, the manual checklist must be signed, and the dogfood transcript must prove attribution behavior.
...
8. **Dogfood fixture insufficiency.** `runs/artagents_logo_v3` might be missing enough provenance for `data_quality >= 0.6`.
...
| CLI plumbing | `tests/test_threads_cli_plumbing.py` | 2 | Both `python3 -m artagents executors run` and `python3 -m artagents orchestrators run` accept `--thread <id|@new|@none>`, `--variants N`, and `--from <run-id>:<n>` and pass them into the request objects without altering tool args after `--`. |
...
| Thread CLI lifecycle | `tests/test_threads_cli.py`, `tests/test_threads_lifecycle.py` | 3-4 | `thread new`, `thread list`, `thread show`, `thread archive`, and `thread reopen` work against `.artagents/threads.json`; lazy lifecycle enforcement has no daemon dependency. |
...
| Modality registry | `tests/test_modalities_registry.py` | 6 | Exactly `image_grid`, `audio_waveform`, and `generic_card` are registered; `generic_card` is last in fallback order; `python3 -m artagents modalities {list, inspect}` reports the declarations. |
| Iteration prepare | `tests/test_iteration_prepare.py`, `tests/test_iteration_prepare_cap.py`, `tests/test_iteration_prepare_cache.py` | 6-7 | `iteration.prepare` walks provenance-graph ancestry, computes OQ-6 data quality without penalizing valid roots, enforces SD-043 before uncached summarize dispatch, and writes/reads `.artagents/iteration_cache/<run_id>__<summarizer_model_version>.json`. |
...
## Dogfood Plan: `runs/artagents_logo_v3`
...
Run `python3 -m artagents thread backfill runs/artagents_logo_v3` after brief snapshotting, provenance support, variants, and path-based privacy land.

Acceptance:

- A thread is created or reused for `runs/artagents_logo_v3` with a stable ULID and repo-relative paths only.
...
- Private-path handling is tested by placing one copied fixture note under `runs/artagents_logo_v3/private/` and verifying only sha/kind metadata is emitted.
...
- Assert the operating agent ran `python3 -m artagents thread show @active` before `iteration_video inspect` or render.
...
| Inspect footer | 3 | `executors inspect` and `orchestrators inspect` output | Footer names the active thread and points to `python3 -m artagents thread show @active` for details. |
...
(e) `runs/artagents_logo_v3` dogfood produces all five SD-022 outputs: `iteration.mp4`, `iteration.timeline.json`, `iteration.manifest.json`, `iteration.report.html`, and `iteration.quality.json`.
...
(h) The operating agent ran `python3 -m artagents thread show @active` before `iteration_video inspect` or render.
