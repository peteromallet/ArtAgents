# Reigh ArtAgents Timeline Safety Plan

## Goals

Make the Reigh app <-> ArtAgents timeline boundary safe enough for active product work without turning it into a broad rewrite. The plan is about persisted JSON parity, lossy-round-trip prevention, and clear validation boundaries across:

- `../reigh-app`
- `/Users/peteromalley/Documents/reigh-workspace/ArtAgents`
- `/Users/peteromalley/Documents/banodoco-workspace/packages/timeline-schema`

The practical goal is that a timeline saved by Reigh, inspected or transformed by ArtAgents, and loaded back by Reigh preserves the top-level contract fields that both sides know how to carry, even when one side cannot fully render every visual feature.

## Non-Goals

- Do not port the full Reigh renderer into ArtAgents.
...
- Do not make ArtAgents accept every timeline that Reigh can visually render; ArtAgents only needs to preserve the persisted shape and smoke-load renderable subsets.
...
- **ArtAgents owner**: `/Users/peteromalley/Documents/reigh-workspace/ArtAgents`, especially `artagents/timeline.py`, generated Remotion types, and Remotion smoke checks
...
This distinction lets Reigh keep no-theme default timelines while ArtAgents and theme resolvers continue to reject no-theme inputs at render time.
...
- ArtAgents timeline validation and fallback shared types live in `artagents/timeline.py`. The file describes `banodoco_timeline_schema` as the canonical shape check, then adds ArtAgents-only validation such as effect registry checks.
- ArtAgents generated Remotion shape snapshots include `remotion/src/types.generated.ts` and `remotion/__smoke__/bundle.mjs`.

## Known Drift

- **`generation_defaults` drift**: Reigh treats `generation_defaults` as an optional top-level persisted field in `../reigh-app/src/tools/video-editor/types/index.ts` and preserves it in `../reigh-app/src/tools/video-editor/lib/serialize.ts`. The shared schema in `/Users/peteromalley/Documents/banodoco-workspace/packages/timeline-schema/typescript/src/schemas.ts` does not currently include it, and ArtAgents top-level allowlists in `artagents/timeline.py` / generated Remotion allowlists do not include it.
- **`theme` optionality drift**: Reigh allows and creates persisted timelines without `theme` in `../reigh-app/src/tools/video-editor/types/index.ts` and `../reigh-app/src/tools/video-editor/lib/defaults.ts`. The shared schema currently requires `theme`, while ArtAgents `validate_timeline` requires `Timeline.theme` to be a non-empty slug for renderable timelines.
- **Renderer scope drift**: Reigh's renderer is broader than the ArtAgents local Remotion renderer. ArtAgents Remotion should be treated as a compatibility smoke renderer for loadability and contract preservation, not as a full visual parity target.

## Settled Decisions

- **SD-001** — Make the shared package the persisted JSON contract. _load_bearing: true_
  Rationale: The contract needs one neutral home that both Reigh and ArtAgents can validate against without inheriting either app's renderer assumptions.
...
- **SD-005** — Limit ArtAgents renderer scope to smoke/loadability parity. _load_bearing: true_
...
- ArtAgents render validation may require a non-empty `theme` even though shared persisted validation allows no-theme timelines.

This means a no-theme timeline can be valid persisted JSON while still being non-renderable in a themed ArtAgents path. That is intentional.

## Renderer Scope

ArtAgents local Remotion checks should prove that timelines using the supported subset can be loaded, typed, smoke-rendered, and round-tripped without dropping contract fields. They should not be used as a claim that ArtAgents visually matches Reigh's renderer.

Success for the renderer boundary is smoke/loadability:

- Generated ArtAgents/Remotion allowlists know about the same persisted top-level fields.
- A themed fixture with `theme_overrides` and `generation_defaults` can pass through ArtAgents without losing those fields.
- No-theme persisted fixtures can validate at the shared-schema layer while still failing explicit ArtAgents renderability checks.

## Shared Schema Implementation

This is the first required implementation phase because every downstream check should inherit one persisted JSON contract instead of copying Reigh-specific or ArtAgents-specific assumptions.
...
- No resolver, renderer, or ArtAgents-specific renderability rule is weakened by the persisted-schema change.
...
ArtAgents should align with the shared persisted contract for field preservation while keeping its own renderability gate stricter. This phase is not about making every Reigh timeline render locally. It is about ensuring ArtAgents does not reject or drop top-level persisted fields that Reigh already writes.

Required changes in `/Users/peteromalley/Documents/reigh-workspace/ArtAgents`:

- Add `generation_defaults` to `_TIMELINE_TOP_ALLOWED` in `artagents/timeline.py`.
- Add `generation_defaults: dict[str, Any]` to the fallback `SharedTimelineConfig` `TypedDict` in `artagents/timeline.py`, so ArtAgents still carries the field when `banodoco_timeline_schema` is not importable or is temporarily stale in a local environment.
...
- `timeline.validate_timeline(config)` must continue to require `Timeline.theme` to be a non-empty slug for ArtAgents-renderable timelines.
- A no-theme persisted fixture may pass shared-schema validation, but it should still fail ArtAgents `validate_timeline` with `Timeline.theme must be a non-empty slug`.
- A themed fixture containing `theme`, `theme_overrides`, and `generation_defaults` should pass ArtAgents validation.
- ArtAgents should preserve `generation_defaults` losslessly when reading, validating, normalizing, or writing a themed timeline. Preservation means the exact JSON object survives; ArtAgents does not need to understand its inner keys.

Recommended ArtAgents tests:
...
- Add a rejection test showing the same valid persisted no-theme fixture from the shared schema section still fails `timeline.validate_timeline(...)` in ArtAgents.
- Add or extend a round-trip-style test that serializes a themed timeline through the ArtAgents JSON path and asserts `generation_defaults` is byte-equivalent after reload.

Do not do these in this phase:

- Do not relax ArtAgents `validate_timeline` to allow no-theme render inputs.
...
- Do not port Reigh renderer behavior to make more fixtures visually render in ArtAgents.

Success criteria for this phase:

- ArtAgents accepts and preserves `generation_defaults` on themed timelines.
- ArtAgents still rejects no-theme timelines at the renderability boundary.
- ArtAgents does not inspect or reject unknown inner `generation_defaults` keys.
...
After ArtAgents accepts `generation_defaults` in `artagents/timeline.py`, update the generated Remotion artifacts that mirror ArtAgents timeline types and allowlists. This phase is mechanical but important because stale generated files make the boundary look unsafe even after the Python source is correct.
...
Preferred command path from `/Users/peteromalley/Documents/reigh-workspace/ArtAgents`:
...
- Run `python3 scripts/gen_remotion_types.py` once after the ArtAgents source change.
...
- If `npm run smoke` loads example timelines, keep those fixtures themed and within the local ArtAgents renderer subset. This is a loadability smoke check, not a visual parity assertion against Reigh.
...
- `timeline-no-theme-minimal`
  Assertion: validates as persisted JSON in `timeline-schema`; fails explicit theme resolution; fails ArtAgents render validation; remains omitted by Reigh defaults.
...
- `timeline-themed-with-extras`
  ...
  Assertion: validates in the shared schema, passes ArtAgents validation when otherwise renderable, survives Reigh serializer/data/active save or history flow, and preserves the exact `generation_defaults` JSON object.
...
- `artagents-themed-pass-through`
  Assertion: ArtAgents accepts a themed timeline with arbitrary nested `generation_defaults`, writes or reloads it without loss, and does not inspect inner keys.
...
- **Risk: Persisted validation and renderability get conflated.**
  Mitigation: Pair every no-theme acceptance test with resolver/render rejection tests. The same fixture should pass shared persisted validation and fail `resolveTheme`, `resolve_theme`, and ArtAgents `validate_timeline`.
...
- **Risk: Generated ArtAgents Remotion artifacts drift from Python source.**
...
- **Risk: ArtAgents renderer failures are mistaken for contract failures.**
  Mitigation: Keep renderer smoke fixtures themed and within the ArtAgents-supported subset. Treat broader Reigh renderer coverage as out of scope.
...
- Do not port Reigh renderer behavior into ArtAgents for this plan.
...
2. Update ArtAgents persisted-contract parity.

   Checkpoint: `artagents/timeline.py` accepts `generation_defaults` in top-level allowlists and fallback typed shape, preserves it on themed timelines, and still rejects no-theme render inputs.

3. Regenerate ArtAgents Remotion type and smoke snapshots.
...
In `/Users/peteromalley/Documents/reigh-workspace/ArtAgents`:
...
- ArtAgents validation tests prove themed pass-through works and no-theme render validation still fails.
...
- Theme resolution remains strict in TypeScript, Python, and ArtAgents renderability checks.
...
- ArtAgents accepts and preserves `generation_defaults` for themed timelines without interpreting its inner keys.
- ArtAgents generated Remotion types and smoke snapshots match the Python allowlists.
- No-theme Reigh defaults remain unchanged.
- The plan does not require visual parity between Reigh's renderer and ArtAgents Remotion.
...
- The document lives at `docs/reigh-artagents-timeline-safety-plan.md`.
- The settled theme policy is unchanged: persisted timelines may omit `theme`; theme resolution and ArtAgents renderability still require a non-empty theme.
- Required work is scoped to schema parity, ArtAgents allowlist/generated parity, Reigh round-trip assertions, and focused validation.
...
- ArtAgents Remotion is described only as a compatibility smoke/loadability renderer.
