...
def _resolve_theme_path(theme_path: Path) -> Path:
    ...
    if theme_path.exists():
        return theme_path
    return Path(__file__).resolve().parents[1] / "themes" / str(theme_path) / "theme.json"
...
def _regenerate_primitive_registries(project_dir: Path, theme_path: Path | None) -> None:
    generator = Path(__file__).resolve().parent / "scripts" / "gen_effect_registry.py"
...
def render(
    ...
    *,
    ...
    project_dir = project_dir or (Path(__file__).resolve().parent / "remotion")
    ...
    try:
        ...
        resolved_theme = theme_path or (Path(__file__).resolve().parents[1] / "themes" / "banodoco-default" / "theme.json")
...
def main() -> int:
    ...
    parser.add_argument("--project-dir", type=Path, default=Path(__file__).resolve().parent / "remotion")
    parser.add_argument("--composition", default="TimelineComposition")
    parser.add_argument(
        "--theme",
        type=Path,
        default=Path(__file__).resolve().parents[1] / "themes" / "banodoco-default" / "theme.json",
    )
