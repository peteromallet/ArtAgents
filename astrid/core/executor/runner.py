...
def _run_executor_inner(request: ExecutorRunRequest, executor: ExecutorDefinition) -> ExecutorRunResult:
    ...
    if executor.kind == "built_in" and "pipeline_step" in executor.metadata:
        # In-process dispatch lives inside the hype orchestrator package now
        # (Sprint 9 Phase 2 Step 6.7); imported lazily so the runner does not
        # depend on the builtin pack at module load time. Phase 4 retires this
        # branch entirely once every builtin executor declares runtime argv.
        from astrid.packs.builtin.orchestrators.hype._pipeline import run_builtin_executor

        return run_builtin_executor(executor, request)
...
def build_executor_command(request: ExecutorRunRequest, registry: ExecutorRegistry | None = None) -> tuple[str, ...]:
    ...
    if condition_result.skipped:
        return ()
    if executor.kind == "built_in" and "pipeline_step" in executor.metadata:
        from astrid.packs.builtin.orchestrators.hype._pipeline import (
            _step_for_executor,
