...
Generates an ai-toolkit (ostris) job config from the dataset manifest + vocabulary, writes a `bootstrap.sh`, drops `/workspace/config.yaml` on the pod, uploads the manifest-listed dataset clips + caption sidecars, and starts the AI Toolkit UI on port 8675. Defaults bake in LTX 2.3, resolution buckets `[512, 768]`, 121 frames @ 24fps (5s @ `frames=sec*24+1`), LR 2e-5, rank 32, save/sample every 250 steps, bucketing on, `cache_latents_to_disk: true`. The trigger token `seinfeld scene` is prepended at inference and training via the config's `trigger_word`; on-disk caption files are not mutated. In `--dry-run`, no pod is required and the executor prints the dataset upload it would perform.

Defaults reconcile hivemind (Kijai rank 32, LR 2e-5) with Ostris's own LTX-2.3 character LoRA recipe — see `TRAINING_PLAN.md § Ostris tutorial findings` for the deltas (multi-bucket resolution, latent caching, 5s frame count).
