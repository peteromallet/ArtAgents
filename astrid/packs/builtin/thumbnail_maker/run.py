...
def _write_candidate_contact_sheet(candidates: list[dict[str, Any]], out_path: Path) -> dict[str, Any]:
    ...
    try:
        from astrid.packs.builtin.visual_understand.run import _build_contact_sheet
...
def _load_or_create_scenes(video_path: Path, layout: dict[str, Path]) -> tuple[list[dict[str, Any]], Path, bool]:
    from astrid.packs.builtin.scenes import run as scenes_module
...
def _load_or_create_shots(
    ...
    from astrid.packs.builtin.shots import run as shots_module
...
def _run_visual_selection(
    ...
    if not candidates:
        return None
    try:
        from astrid.packs.builtin.visual_understand import run as visual_understand
...
def _run_image_generation(
    ...
    from astrid.packs.builtin.generate_image import run as generate_image
