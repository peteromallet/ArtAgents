...
This executor wraps `astrid.packs.builtin.executors.generate_image.run` and expects a prompt file. Put one
...
- `saint-peter-of-banodoco` — onboarding portrait of the maker as Saint Peter
  ...
  ```bash
  python3 -m astrid.packs.builtin.executors.generate_image.run \
