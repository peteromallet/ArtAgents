...
round-trips through ``artagents.core.task.plan.load_plan``), and writes the
...
DEFAULT_PACKS_ROOT = REPO_ROOT / "artagents" / "packs"
...
def _load_module_isolated(module_path: Path, qualified_id: str):
    unique = f"_artagents_orchestrate_{qualified_id.replace('.', '_')}_{uuid.uuid4().hex}"
