"""ArtAgents banodoco worker pool implementation.
...
* **Contract #1 — claim-next-task body**:
  ...
  ``{worker_id, run_type:"banodoco-worker", worker_pool:"banodoco",
    task_types:[...]}``. AA's :mod:`artagents.core.reigh.task_client` mirrors
...
* **Contract #4 — update-task-status with result_data**: reigh-app commit
  ...
  ``{correlation_id}`` on failure via :mod:`artagents.core.reigh.task_client`'s
...
from artagents.core.reigh.timeline_io import Mutator, SaveResult, TimelineConfig
from artagents.core.reigh.worker_jwt import JwtVerificationError, VerifiedJwt, verify_user_jwt
...
def main(argv: list[str] | None = None) -> int:
    """``python3 -m artagents worker --pool banodoco`` entrypoint."""

    import argparse

    parser = argparse.ArgumentParser(prog="python3 -m artagents worker")
