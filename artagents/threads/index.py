"""Locked atomic storage for `.artagents/threads.json`."""
...
class ThreadIndexStore:
    def __init__(self, repo_root: Path | str = REPO_ROOT, *, lock_timeout: float = LOCK_TIMEOUT_SECONDS) -> None:
        ...
        self.state_dir = self.repo_root / ".artagents"
            ...
            try:
                ...
                if time.monotonic() >= deadline:
                    raise ThreadIndexLockTimeout(
                        "Timed out waiting for .artagents/threads.json lock. "
                        "Another ArtAgents process may still be writing thread state; "
                        "wait for it to finish or remove the stale lock file only after verifying no ArtAgents process is running."
                    ) from exc
                time.sleep(0.05)
