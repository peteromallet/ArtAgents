...
State path: ``$XDG_STATE_HOME/artagents/skills.json`` if XDG_STATE_HOME is set,
else ``~/.local/state/artagents/skills.json``. This single location works on
...
def state_path() -> Path:
    ...
    if override:
        return Path(override) / "artagents" / "skills.json"
    xdg = os.environ.get("XDG_STATE_HOME")
    base = Path(xdg) if xdg else Path.home() / ".local" / "state"
    return base / "artagents" / "skills.json"
