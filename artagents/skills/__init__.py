...
``artagents/packs/<pack>/skill/SKILL.md`` with Claude-style frontmatter.
"""
...
def nudge_if_needed(*, argv: list[str], state_path: Path | None = None, stream=sys.stderr) -> bool:
    ...
    message = (
        f"[artagents] Tip: install the skills layer for {pretty}: "
        f"python3 -m artagents skills install --all   (suppress: {NUDGE_ENV}=1)"
    )
