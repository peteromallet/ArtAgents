...
```bash
python3 -m artagents executors run iteration.prepare --out runs/prepare --input target_run_id=<run-id>
```

Direct form:

```bash
python3 -m artagents.executors.iteration_prepare.run --target-run-id <run-id> --out runs/prepare
```
