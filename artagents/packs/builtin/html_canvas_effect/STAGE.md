...
```bash
python3 -m artagents executors inspect builtin.html_canvas_effect --json
```

Dry-run through the canonical executor CLI:

```bash
python3 -m artagents executors run builtin.html_canvas_effect \
...
```bash
python3 -m artagents.packs.builtin.html_canvas_effect.run \
...
- `artagents/packs/local/elements/effects/<effect-id>/component.tsx`
- `artagents/packs/local/elements/effects/<effect-id>/element.yaml`
- report JSON at `--out`
...
```bash
python3 -m artagents.packs.builtin.render.run \
...
metadata. Current ArtAgents render should remain the default path, but using the
