...
def step_tile(args: argparse.Namespace, out: Path) -> Path:
    cmd = [
        sys.executable, "-m", "artagents.packs.builtin.tile_video.run",
...
def _visual_understand_query(image: Path, query: str, env_file: Path | None,
    ...
    cmd = [
        sys.executable, "-m", "artagents.packs.builtin.visual_understand.run",
        "--image", str(image),
        "--query", query,
...
def _foley_one(clip: Path, prompt: str, out_audio: Path, env_file: Path | None,
                dry_run: bool) -> None:
    cmd = [
        sys.executable, "-m", "artagents.packs.external.fal_foley.run",
        "--clip", str(clip),
        "--prompt", prompt,
        "--out", str(out_audio),
    ]
...
def step_review(out: Path, enriched: dict[str, Any]) -> Path:
    ...
    cmd = [
        sys.executable, "-m", "artagents.packs.builtin.foley_review.run",
...
def step_page(out: Path) -> Path:
    page_dir = out / "page"
    cmd = [
        sys.executable, "-m", "artagents.packs.builtin.spatial_audio_page.run",
