---
name: "artagents"
short_description: "ArtAgents — file-based toolkit for agents to make video, image, and audio art alongside a human."
...
python3 -m artagents [executors|orchestrators|elements] search <terms>
```

If you don't know which tool to use, run `python3 -m artagents <kind> search <terms>` first — don't guess from id alone.
...
ArtAgents ships its own skills layer for the three supported agent harnesses (Claude Code, Codex, Hermes). One command installs the `_core` skill plus any per-pack skills into every harness it detects on the machine:

```bash
python3 -m artagents skills list                 # show installable packs and current per-harness state
python3 -m artagents skills install --all        # install every pack into every detected harness
python3 -m artagents skills doctor               # verify symlinks resolve and AGENTS.md block parses
python3 -m artagents skills uninstall _core      # remove from all harnesses
```

Default mechanism is per-pack symlinks under `~/.claude/skills/`, `~/.codex/skills/`, and `${HERMES_HOME:-~/.hermes}/skills/`. Codex additionally maintains an idempotent fenced block in `~/.codex/AGENTS.md` listing each installed pack. Hermes accepts an opt-in `--mechanism external-dir` that registers the whole `artagents/packs` tree via `~/.hermes/config.yaml` `skills.external_dirs` instead of per-pack symlinks.

If no harness is installed, ArtAgents prints a one-line nudge to stderr at most once every seven days when you run a non-`skills` subcommand. Suppress with `ARTAGENTS_NO_NUDGE=1` or `--quiet`.
...
- Python timeline IO + validation: `artagents/timeline.py`
- Render entrypoint: `artagents/packs/builtin/render/run.py`

### Available elements

Run `python3 -m artagents elements list` (or `inspect <kind> <id> --json` for a single element). At time of writing:
...
```bash
python3 -m artagents elements fork effects text-card
# edit artagents/packs/local/elements/effects/text-card/component.tsx
